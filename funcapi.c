...
// Hash函数，将一个bigram的两个字符映射到0~65535的空间
...
Datum jaccard_index(PG_FUNCTION_ARGS) {
	//elog(LOG, "jaccard_index triggered.");
	
	// SIZE_A、SIZE_B分别记录A和B有多少个不重复的bigram，SIZE_C记录A和B有多少个不重复的公共bigram
	int i, j, k, SIZE_A, SIZE_B, SIZE_C;
	// 用2048个int(32位)作为一个Bitmap来容纳字符集大小平方的空间(255^2 = 65536)
	...
	// 将A的所有bigram加入Bitmap
	MAP_SET(MAP_A, IDX('$', A[0]));
	MAP_SET(MAP_A, IDX(A[N - 1], '$'));
	SIZE_A = 2;
	for (i = 0; i < N - 1; ++ i) {
		k = IDX(A[i], A[i + 1]);
		if (!MAP_CHK(MAP_A, k)) {
	...
	for (i = 0; i < M - 1; ++ i) {
		k = IDX(B[i], B[i + 1]);
		// 当前bigram在A中出现过，且在B中是第一次出现
		if (!MAP_CHK(MAP_B, k) && MAP_CHK(MAP_A, k)) {
			++ SIZE_C;
			//elog(LOG, "%common gram #%d: %c%c", SIZE_C, B[i], B[i + 1]);
		}
		if (!MAP_CHK(MAP_B, k)) {
			++ SIZE_B;
			MAP_SET(MAP_B, k);
	...
	//elog(LOG, "SIZE_A = %d, SIZE_B = %d, SIZE_C = %d", SIZE_A, SIZE_B, SIZE_C);
	
	// 根据公式计算Jaccard Index
	float RET = (float)(SIZE_C) / (SIZE_A + SIZE_B - SIZE_C);
	
	//elog(LOG, "j-index = %f", RET);
...
Datum levenshtein_distance(PG_FUNCTION_ARGS) {
	//elog(LOG, "levenshtein_distance triggered.");
	...
	//elog(LOG, "lev dist = %d", DIST[N][M]);
	
	PG_RETURN_INT64(DIST[N][M]);
}
