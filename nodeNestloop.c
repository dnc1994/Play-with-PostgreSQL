...
ExecNestLoop(NestLoopState *node)
{
	...
	for (;;)
	{
		...
		{
			ENL1_printf("getting new outer tuple (from block)");
			
			//elog(LOG, "block nested loop triggered.");
			...
			if (TupIsNull(outerTupleSlot)) {
				...
				if (econtext->ecxt_outer_block_head) {
					TupleTableSlot **ptr = (TupleTableSlot **)econtext->ecxt_outer_block_head;
					void *tmp = ptr;
					...
					// 上个Block的实际Tuple数量不足BLOCK_SIZE个，说明Outer Tuple已取完
					if (econtext->ecxt_outer_block_ptr - (TupleTableSlot **)econtext->ecxt_outer_block_head != BLOCK_SIZE) {
						return NULL;
					}
				}
				...
				// 没有下个Block了，说明Outer Tuple已取完，结束JOIN
				if (!econtext->ecxt_outer_block_ptr) {
					return NULL;
				}
				// 记录新Block的起始地址
